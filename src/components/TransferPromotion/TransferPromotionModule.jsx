...
export const TransferPromotionModule = () => {
  ...
  const [formData, setFormData] = useState({
    request_type: 'Spouse Ground',
    target_district: 'Nalanda',
    target_school: 'Govt Multipurpose High School, Bihar Sharif',
    reason: ''
  });
  ...
  const handleTransferSubmit = (e) => {
    ...
    setFormData({ request_type: 'Spouse Ground', target_district: 'Nalanda', target_school: '', reason: '' });
  };
            ...
            >
              ...
              <div className="p-3 rounded-xl bg-purple-50/50 border border-purple-100 space-y-1 text-xs text-slate-700">
                <div className="flex justify-between">
                  ...
                  <strong className="text-slate-900">{trf.current_district}</strong>
                </div>
                <div className="flex justify-between">
                  <span>Requested District:</span>
              ...
              <div className="pt-2 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {role === 'admin' && trf.status !== 'Approved' && (
                    <button
                      onClick={() => updateTransferStatus(trf.id, 'Approved', 'Transfer order generated', true)}
                      className="px-3 py-1 rounded-lg bg-emerald-600 text-white text-xs font-extrabold"
                    >
      ...
      {activeSubTab === 'promotions' && (
        <div className="bg-white p-6 rounded-2xl border border-purple-100 shadow-sm space-y-4">
          <h3 className="text-sm font-extrabold text-purple-950 flex items-center gap-2">
            ...
            Seniority & Promotion Matrix 2026
          </h3>
            ...
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-purple-100 text-slate-700">
                {teachers.map((tch, idx) => {
                  const score = (tch.experience_years * 5) + (tch.tet_score / 10) + 15;
                  ...
                  return (
                    <tr key={tch.id} className="hover:bg-purple-50/50">
                      ...
                      <td className="p-3">{tch.experience_years} Years</td>
                      <td className="p-3 font-extrabold text-emerald-700">{tch.tet_score} / 150</td>
                      <td className="p-3">
                      ...
                      <td className="p-3">
                        <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-extrabold ${
                          ...
                          {isEligible ? 'Eligible for PGT' : 'Ineligible'}
                        </span>
                      </td>
                      <td className="p-3 text-right">
                        {isEligible && (
                          <button 
                            onClick={() => alert(`Promotion Order issued for ${tch.full_name}!`)}
            ...
            <form onSubmit={handleTransferSubmit} className="space-y-3">
              <div>
                ...
                >
                  ...
                  <option value="General Cadre Transfer">General Cadre Transfer</option>
                </select>
              </div>

              <div>
                <label className="text-xs font-bold text-slate-700">Target District</label>
                <select
              ...
              <div>
                <label className="text-xs font-bold text-slate-700">Target School Preference</label>
                <input
                  type="text"
                  placeholder="e.g. PM Shri Govt HSS, Sector 4"
      ...
      {selectedTransferForOrder && (
        <div className="fixed inset-0 z-50 bg-purple-950/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-lg bg-white border border-purple-100 rounded-3xl p-6 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between border-b border-purple-100 pb-3">
              <h3 className="text-base font-extrabold text-purple-950">Official State Transfer Order</h3>
            ...
            <div className="p-5 rounded-2xl bg-purple-50/60 border border-purple-100 text-slate-900 space-y-3 font-sans text-xs">
              <div className="text-center border-b border-purple-200 pb-2">
                <p className="font-extrabold text-sm uppercase text-purple-950">Government Education Directorate</p>
                <p className="text-[10px] text-purple-800 font-mono">Official Transfer Order No: TRF-2026-9041</p>
              </div>
              ...
              <div className="bg-white p-3 rounded-xl border border-purple-200 space-y-1 font-medium">
                ...
                <p><strong>Posted To:</strong> {selectedTransferForOrder.target_school || 'District Reserve Pool'} ({selectedTransferForOrder.target_district})</p>
              </div>
            </div>
