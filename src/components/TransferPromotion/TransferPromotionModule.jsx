            ...
            visibleTransfers.map((trf) => {
              ...
              return (
                ...
                <div className="pt-2 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    {isPrivilegedRole && trf?.status !== 'Approved' && trf?.status !== 'Rejected' && (
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => updateTransferStatus(trf.id, 'Approved', 'Approved & Transferred by HOD / Dean', true)}
                          className="px-3 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-extrabold shadow-sm transition-all flex items-center gap-1"
                        >
                          <Check className="w-3.5 h-3.5 stroke-[3]" />
                          Approve & Transfer
                        </button>
                        <button
                          onClick={() => updateTransferStatus(trf.id, 'Rejected', 'Rejected by HOD / Dean', false)}
                          className="px-2.5 py-1.5 rounded-xl border border-rose-200 text-rose-700 hover:bg-rose-50 text-xs font-bold transition-all"
                        >
                          Reject
                        </button>
                      </div>
                    ...
                    {trf?.status === 'Approved' && (
                      <div className="flex items-center gap-2">
                        <span className="px-2.5 py-1 rounded-lg text-[10px] font-extrabold bg-emerald-100 text-emerald-900 border border-emerald-200">
                          Posting Updated
                        </span>
                        <button
                          onClick={() => setSelectedTransferForOrder(trf)}
                          className="px-3 py-1.5 rounded-xl bg-purple-100 hover:bg-purple-200 text-purple-900 text-xs font-extrabold flex items-center gap-1 transition-all"
                        >
                    ...
                    {trf?.status === 'Rejected' && (
                      <span className="px-2.5 py-0.5 rounded-full text-[10px] font-extrabold bg-rose-100 text-rose-900 border border-rose-200">
